...
const AllMyHobbies = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllMyHobbiesQuery(query);
  const [deleteMyHobby] = useDeleteMyHobbyMutation();

  // Filters companies based on the search query
  const filteredData = data?.data?.data;

  const handleDeleteMyHobby = async (homeHero) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await deleteMyHobby(homeHero?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
      ...
      <div className="bg-[#0D0E12] sticky top-[75px] md:top-[82px] z-[450] py-2 md:py-0">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 ">
          <div className="w-full">
            <SectionTitle
              big_title={"All My Hobbies"}
              link_one={"/"}
              title_one={"My Hobbies"}
              title_two={"All My Hobbies"}
          ...
          <div className="flex items-center w-full justify-between md:justify-end gap-4">
            ...
            <Link href="/about-page/my-hobby/add-my-hobby">
              <button className="btn w-[150px] md:w-64">Add My Hobby</button>
            </Link>
          </div>
        </div>
      </div>
      <div className="mx-auto w-full pt-2 mt-6">
        <h1 className="table_header">All MyHobbies</h1>
                    ...
                    <tr key={index} className="tbody_tr">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-2">
                          <EditIcon
                            edit_link={`/about-page/my-hobby/edit-my-hobby/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteMyHobby}
                            item={item}
                          />
                        </div>
                      </td>
