...
} from "@/redux/api/snapshotCategoryApi";

const AllSnapshotCategories = () => {
  const [searchValue, setSearchValue] = useState({
    search: "",
  });
  const [searchQuery, setSearchQuery] = useState("");
  const { data, error, isLoading, refetch } =
    useGetAllSnapshotsCategoriesQuery(searchValue);
  const [deleteSnapshotCategory] = useDeleteSnapshotsCategoryMutation(); // ✅ Correct
  ...
  const handleDeleteSnapshotCategory = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All SnapshotCategory "${faq?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteSnapshotCategory(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllSnapshotCategory "${faq?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        <div>
          <SectionTitle
            big_title={"All SnapshotCategory"}
            link_one={"/"}
            title_one={"Home"}
            link_two={"/about-page/snapshot-category/all-snapshot-categories"}
            title_two={"All SnapshotCategory"}
            title_three={"Add SnapshotCategory"}
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/about-page/snapshot-category/add-snapshot-category">
            <button className="btn w-64">Add Snapshot Category</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Snapshot Category</h1>
          <div className="table_section">
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  <th className="table_th w-4">#SL</th>
                  <th className="table_th ">Snapshot Category Title</th>
                    ...
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/about-page/snapshot-category/edit-snapshot-category/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteSnapshotCategory}
                            item={item}
                          />
                        </div>
                      </td>
...
export default AllSnapshotCategories;
