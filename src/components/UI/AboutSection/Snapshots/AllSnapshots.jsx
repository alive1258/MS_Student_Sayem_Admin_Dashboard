...
const AllSnapshots = () => {
  ...
  const { data, error, isLoading, refetch } =
    useGetAllSnapshotsQuery(searchValue);
  ...
  const handleDeleteSnapshot = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All Snapshot"${faq?.snapshotsCategory?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteSnapshot(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The Snapshot "${faq?.snapshotsCategory?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/about-page/snapshots/add-snapshot">
            <button className="btn w-64">Add Snapshot</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Snapshot</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  ...
                  <th className="table_th ">Snapshot Category</th>
                    ...
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/about-page/snapshots/edit-snapshot/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteSnapshot}
                            item={item}
                          />
                        </div>
                      </td>
