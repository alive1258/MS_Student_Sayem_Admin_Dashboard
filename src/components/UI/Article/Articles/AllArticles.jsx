...
const AllArticles = () => {
  ...
  const { data, error, isLoading, refetch } =
    useGetAllArticlesQuery(searchValue);
  ...
  const handleDeleteArticle = async (faq) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await deleteArticle(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
    ...
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/articles/articles/add-article">
            <button className="btn w-64">Add Article</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Article</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  ...
                  <th className="table_th ">Article Category</th>
                  ...
                  filteredData?.map((item, index) => (
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>
                          {truncateCharacters(item?.articleCategory?.name, 30)}
                        </p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.article_title, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>
                          {truncateCharacters(item?.article_description, 30)}
                        </p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.article_tags, 30)}</p>
                      </td>
                      <td className="table_th">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/articles/articles/edit-article/${item?.id}`}
                          />
                          <DeleteIcon
