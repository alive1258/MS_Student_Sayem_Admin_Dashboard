...
const AllSectionDescription = () => {
  const [searchValue, setSearchValue] = useState({
    search: "",
  });
  const [searchQuery, setSearchQuery] = useState("");
  const { data, error, isLoading, refetch } =
    useGetAllSectionDescriptionsQuery(searchValue);
  const [deleteSectionDescription] = useDeleteSectionDescriptionMutation();
  ...
  const handleDeleteSectionDescription = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All SectionDescriptions "${faq?.faculty_name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteSectionDescription(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllSectionDescriptions "${faq?.faculty_name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        <div>
          <SectionTitle
            big_title={"All SectionDescription"}
            link_one={"/"}
            title_one={"Home"}
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/section-description/add-section-description">
            <button className="btn w-64">Add SectionDescription</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All SectionDescription</h1>
                    ...
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/section-description/edit-section-description/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteSectionDescription}
                            item={item}
                          />
                        </div>
                      </td>
...
export default AllSectionDescription;
