...
const AllProjects = () => {
  ...
  const { data, error, isLoading, refetch } =
    useGetAllProjectsQuery(searchValue);
  ...
  const handleDeleteProject = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All Project"${faq?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteProject(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllProject "${faq?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/projects/project/add-project">
            <button className="btn w-64">Add Project</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Project</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  ...
                  <th className="table_th ">Project Category</th>
                  <th className="table_th "> Title</th>
                  <th className="table_th "> Description</th>
                  <th className="table_th "> Tags</th>
                  <th className="table_th ">Publish Time</th>
                  ...
                  filteredData?.map((item, index) => (
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>
                          {truncateCharacters(item?.projectCategory?.name, 30)}
                        </p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.project_title, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>
                      ...
                      <td className="table_th">
                        <p>{truncateCharacters(item?.project_tags, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.publish_time, 30)}</p>
                      </td>

                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/projects/project/edit-project/${item?.id}`}
                          />
                          <DeleteIcon
