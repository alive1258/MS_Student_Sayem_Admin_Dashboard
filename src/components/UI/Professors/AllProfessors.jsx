...
const AllProfessors = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllProfessorsQuery(query);
  const [deleteAboutMe] = useDeleteProfessorMutation();

  // Filters professor based on the search query
  const filteredData = useMemo(() => data?.data?.data || [], [data]);

  const handleDeleteAboutMe = async (professor) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the professor "${professor?.professor_name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteAboutMe(professor?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The professor "${professor?.professor_name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <div className="bg-[#0D0E12] sticky top-[75px] md:top-[82px] z-[450] py-2 md:py-0">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 ">
          <div className="w-full">
            <SectionTitle
              big_title={"All Professor"}
              link_one={"/"}
              title_one={"Professor"}
              title_two={"All Professor"}
              link_two={"/professors/all-professors"}
            />
          </div>
          ...
          <div className="flex items-center w-full justify-between md:justify-end gap-4">
            ...
            <Link href="/professors/add-professor">
              <button className="btn w-[150px] md:w-64">Add Professor</button>
            </Link>
          </div>
        </div>
      </div>
      <div className="mx-auto w-full pt-2 mt-6">
        <h1 className="table_header">All Professors</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row ">
                  ...
                  <th className="table_th">Professor Name</th>
                  ...
                  filteredData?.map((item, index) => (
                    <tr key={index} className="tbody_tr">
                      ...
                      <td className="table_th ">
                        <p>{truncateCharacters(item?.professor_name, 30)}</p>
                      </td>
                      <td className="table_th ">
                        <p>{truncateCharacters(item?.institute, 30)}</p>
                      </td>
                      <td className="table_th ">
                        <p>{truncateCharacters(item?.department, 30)}</p>
                      </td>
                      <td className="table_th ">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-2">
                          <EditIcon
                            edit_link={`/professors/edit-professor/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteAboutMe}
                            item={item}
                          />
                        </div>
                      </td>
