...
const AllExtraCurriculums = () => {
  const [searchValue, setSearchValue] = useState({
    search: "",
  });
  const [searchQuery, setSearchQuery] = useState("");
  const { data, error, isLoading, refetch } =
    useGetAllExtraCurriculumsQuery(searchValue);
  const [deleteExtraCurriculum] = useDeleteExtraCurriculumMutation();
  ...
  const handleDeleteExtraCurriculum = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All extraCurriculum"${faq?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteExtraCurriculum(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllextraCurriculum "${faq?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        <div>
          <SectionTitle
            big_title={"All extraCurriculum "}
            link_one={"/"}
            title_one={"Home"}
            link_two={
              "/extra-curriculum/extra-curriulums/all-extra-curriculums"
            }
            title_two={"All extraCurriculum"}
            title_three={"Add extraCurriculum"}
            link_three={
              "/extra-curriculum/extra-curriulums/add-extra-curriculum"
            }
          />
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/extra-curriculum/extra-curriulums/add-extra-curriculum">
            <button className="btn w-64">Add Extra Curriculum</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All extraCurriculum</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  ...
                  <th className="table_th ">Extra Curriculum Category</th>
                  <th className="table_th "> Title</th>
                  <th className="table_th "> Description</th>
                  <th className="table_th "> institute</th>
                  <th className="table_th ">organization</th>
                  ...
                  filteredData?.map((item, index) => (
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>
                          {truncateCharacters(
                            item?.extraCurriculumCategory?.title,
                            30
                          )}
                        </p>
                      </td>
                      ...
                      <td className="table_th">
                        <p>{truncateCharacters(item?.institute, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.organization, 30)}</p>
                      </td>

                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/extra-curriculum/extra-curriulums/edit-extra-curriculum/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteExtraCurriculum}
                            item={item}
                          />
                        </div>
                      </td>
...
export default AllExtraCurriculums;
