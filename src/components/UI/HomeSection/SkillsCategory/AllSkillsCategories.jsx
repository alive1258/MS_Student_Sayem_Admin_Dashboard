...
const AllSkillsCategories = () => {
  ...
  const { data, error, isLoading, refetch } =
    useGetAllSkillsCategoriesQuery(searchValue);
  const [deleteSkillsCategory] = useDeleteSkillsCategoryMutation(); // ✅ Correct
  ...
  const handleDeleteSkillsCategory = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All SkillsCategory "${faq?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteSkillsCategory(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllSkillsCategory "${faq?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        <div>
          <SectionTitle
            big_title={"All SkillsCategory"}
            link_one={"/"}
            title_one={"Home"}
            link_two={"/home-page/skills-category/all-skills-categories"}
            title_two={"All SkillsCategory"}
            title_three={"Add SkillsCategory"}
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/home-page/skills-category/add-skills-category">
            <button className="btn w-64">Add Skills Category</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Skills Category</h1>
          <div className="table_section">
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  <th className="table_th w-4">#SL</th>
                  <th className="table_th ">Skills Category Title</th>
                    ...
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/home-page/skills-category/edit-skills-category/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteSkillsCategory}
                            item={item}
                          />
                        </div>
                      </td>
...
export default AllSkillsCategories;
