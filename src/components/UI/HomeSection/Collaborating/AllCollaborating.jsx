...
const AllCollaborating = () => {
  const [searchValue, setSearchValue] = useState({
    search: "",
  });
  const [searchQuery, setSearchQuery] = useState("");
  const { data, error, isLoading, refetch } =
    useGetAllCollaboratingQuery(searchValue);
  const [deleteCollaborating] = useDeleteCollaboratingMutation();
  ...
  const handleDeleteCollaborating = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All Collaborating "${faq?.faculty_name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteCollaborating(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllCollaborating "${faq?.faculty_name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/home-page/collaborating/add-collaborating">
            <button className="btn w-64">Add Collaborating</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Collaborating</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  ...
                  <th className="table_th">Linkedin Link</th>
                  <th className="table_th">Location</th>
                ...
                {filteredData?.length > 0 ? (
                  filteredData?.map((item, index) => (
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>{truncateCharacters(item?.phone, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.linkedin_link, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.location, 30)}</p>
                      </td>

                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/home-page/collaborating/edit-collaborating/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteCollaborating}
                            item={item}
                          />
                        </div>
                      </td>
...
export default AllCollaborating;
