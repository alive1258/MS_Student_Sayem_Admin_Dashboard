...
const AllExperiences = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllExperiencesQuery(query);
  ...
  const handleDeleteExperience = async (experience) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await deleteExperience(experience?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
      ...
      <div className="bg-[#0D0E12] sticky top-[75px] md:top-[82px] z-[450] py-2 md:py-0">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 ">
          <div className="w-full">
            <SectionTitle
              big_title={"All Experience"}
              link_one={"/"}
              title_one={"Experience"}
          ...
          <div className="flex items-center w-full justify-between md:justify-end gap-4">
            ...
            <Link href="/home-page/experience/add-experience">
              <button className="btn w-[150px] md:w-64">Add Experience</button>
            </Link>
          </div>
        </div>
      </div>
      <div className="mx-auto w-full pt-2 mt-6">
        <h1 className="table_header">All Experiences</h1>
                ...
                {filteredData?.length > 0 ? (
                  filteredData?.map((item, index) => (
                    <tr key={index} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>{truncateCharacters(item?.position, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.subject, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.duration, 30)}</p>
                      </td>
                      <td className="table_th">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-2">
                          <EditIcon
                            edit_link={`/home-page/experience/edit-experience/${item?.id}`}
                          />
                          <DeleteIcon
